...
class ObjectFinder:
    ...
    def _load_params(self):
        ...
        self.min_standoff   = rospy.get_param("~min_standoff", 0.31)    # meters
    ...
    def _init_arm(self):
        ...
        self.mc.send_angles([-50.0, 0.0, -10.0, -90.0, 0.0, 57], 50)
    ...
    def amcl_callback(self, msg):
        """
        Saves the start pose of the robot
        """
    ...
    def tick(self, rgb_msg: Image, depth_msg: Image, info_msg: CameraInfo):
        ...
        if self.state == self.S_GRASPED and det is not None:
    ...
    def do_grasp(self, X_arm_mm, Y_arm_mm, Z_arm_mm):
        ...
        try:
            ...
            Z_arm_mm = Z_arm_mm + 13

            self.mc.send_coords([X_arm_mm, Y_arm_mm + 30, Z_arm_mm + 5, rx, ry, rz], speed)

            self.mc.send_coords([X_arm_mm, Y_arm_mm , Z_arm_mm, rx, ry, rz], speed)

            self.mc.set_gripper_state(1, 80)

            self.mc.send_angles([-40.0, -50.0, -40.0, 100.0, -5.0, 52], 20)
            return True
